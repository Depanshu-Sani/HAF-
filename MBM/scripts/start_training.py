...
def main_worker(gpus_per_node, opts):
    ...
    if opts.loss == "cross-entropy" and opts.feature_space != "haf++":
        ...
        def loss_func(logits, labels, m=0):
            ...
            loss_cosine = 1 - torch.abs(F.cosine_similarity(logits[:, torch.where(leaf_node_masks[level - 1] == 1)[0]],
                                                            leaf_node_embeddings[:, torch.where(leaf_node_masks[level - 1] == 1)[0]][labels]))
            # loss_dist = -out[range(labels.shape[0]), labels]
            # loss_norm = torch.clamp(1 - torch.norm(logits[:, torch.where(leaf_node_masks[level - 1] == 1)[0]], dim=1), 0, 1)
            loss = loss_ce.mean() + loss_cosine.mean()
            # import pdb; pdb.set_trace()
            return loss, out
